...
- 默认通过纯 Go ONNX 后端运行 `all-MiniLM-L6-v2`，批量生成 384 维归一化向量，
  保持零出站推理；模型缺失时 macOS 可回退 Natural Language 系统句向量，其他
  平台安全降级为词法搜索。
- 笔记按标题分节和句子边界积累到约 1,000 字符，短尾合并；内容哈希让未改变的
  文档跳过重算，资源 `.reflect.md` 描述也作为引用笔记的语义分块。
- 全局命令面板与 AI Chat 共用 FTS5 + 语义 reciprocal-rank fusion；详情侧栏直接
  比较已存分块向量并显示相似笔记，不重复运行模型。
...
- Chat 打开时会恢复六小时内更新的最近会话；桌面端显示固定历史栏，移动端通过
  顶栏入口打开历史和新对话。
- 本地语义搜索默认关闭；启用后后台增量建立索引并在设置页报告进度、模型、失败和
